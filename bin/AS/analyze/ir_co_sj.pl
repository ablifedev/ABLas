...
my $buf = "-";
my %opts;
GetOptions( \%opts, "as=s","exp=s","name=s","o=s", "h" );
...
print OUT "#Chr\tAss1\tAss2\tStrand\tType\tIsoform";
foreach my $asname(@name_group){
	print OUT "\t$asname\:Ratio\t$asname\:intron_boundary_reads\t$asname\:intron_sj\t$asname\:isfiltered";
}
print OUT "\n";
...
sub load_AS {
	for(my $i=0;$i<=$#as_group;$i++){
		...
		while (<AS>) {              #把所有的junction添加进hash
			#Chr01   8317    9654    -  xx xx     IntronR 0.42    1.31    3.10    PAC:26823939
			chomp;
			next if ( $_ =~ /^#/ );
			my @line = split(/\t/);
			my $key = "$line[0]\t$line[1]\t$line[2]\t$line[3]\t$line[6]\t$line[-1]";
			$as{$key}{"as"}= 1;
			# $filter{$key}{$name_group[$i]}=1;
			$exphash{$key}{$name_group[$i]}="$line[7]\t$line[8]\t$line[9]\t1";
		}
		close AS;
	}
}
...
sub co_AS {
	foreach my $key (sort keys %as){
		...
		for(my $i=0;$i<=$#name_group;$i++){
			$exphash{$key}->{$name_group[$i]}="-\t-\t-\t-" if not defined($exphash{$key}->{$name_group[$i]});
...
sub load_all_exp {
	for(my $i=0;$i<=$#exp_group;$i++){
		...
		while (<EXP>) {
			...
			next if ( $_ =~ /^#/i );
			my @line=split(/\t/);
			my $key="$line[0]\t$line[1]\t$line[2]\t$line[3]\tIntronR\t$line[-1]";
			my $filterflag = defined($filter{$key}{$name_group[$i]}) ? "y" : "n";
			my $value="";
