...
my %nir_s_num = (); #number of sample name
my %nir_list_num = (); #number string of list name
my %nir_list_count = (); #number string of list name

my %ir_s_num = (); #number of sample name
my %ir_list_num = (); #number string of list name
my %ir_list_count = (); #number string of list name
...
sub Get_config_option {
	...
	while (<CONFIG>) {
		...
		if ($line[0] =~ /^sample/i) {
			...
			$nir_s_num{$sample_name}=&select_index($conir,$sample_name) + 1;
			$ir_s_num{$sample_name}=&select_index($coir,$sample_name) + 1;
			...
			$list_samples{$listname} = join("|",@listarray);
			my $firstnum = $nir_s_num{$listarray[0]};
			$nir_list_num{$listname}=$firstnum;
			$firstnum = $ir_s_num{$listarray[0]};
			$ir_list_num{$listname}=$firstnum;
			for(my $i=1;$i<scalar(@listarray);$i++){
				$nir_list_num{$listname}.=",".$nir_s_num{$listarray[$i]};
				$ir_list_num{$listname}.=",".$ir_s_num{$listarray[$i]};
			}
			$nir_list_count{$listname}=scalar(@listarray);
			print "nir list:\n";
			print $listname,"\t",$nir_list_count{$listname},"\t",$nir_list_num{$listname},"\n";
			$ir_list_count{$listname}=scalar(@listarray);
			print "ir list:\n";
			print $listname,"\t",$ir_list_count{$listname},"\t",$ir_list_num{$listname},"\n";
...
sub RAS_calculate {
	...
	foreach my $group (sort keys %{$option_ref->{"GROUP"}}) {
		...
		my $nir_list_num_1 = $nir_list_num{$sample1};
		my $nir_list_num_2 = $nir_list_num{$sample2};
		my $ir_list_num_1 = $ir_list_num{$sample1};
		my $ir_list_num_2 = $ir_list_num{$sample2};
		my $samplelist1 = $list_samples{$sample1};
		my $samplelist2 = $list_samples{$sample2};
		...
		# nir ras
		&make_ras_test_script($nir_list_num_1,$nir_list_num_2,"_conir.r",$dir);
		...
		# ir ras
		&make_ras_test_script($ir_list_num_1,$ir_list_num_2,"_coir.r",$dir);
		...
		`perl /users/ablife/ablife-perl/public/get_sub_file_filter_by_titleitem.pl _$vs\_conir_test_result.txt 'ID|Chr|Ass|Strand|Mss|Type|Isoform|KorN|$samplelist1|$samplelist2|Tvalue|Pvalue' > _$vs\_nir_test_result.txt && cat _$vs\_nir_test_result.txt | perl -ne 'chomp;if(/^X\\.ID/){print \$_,"\\tDiffType\\n";next;}\@line=split(/\\t/);if(\$line[-2]<=$pf && \$line[-1]<0){print \$_,"\\tdown\\n";}if(\$line[-2]<=$pf && \$line[-1]>0){print \$_,"\\tup\\n";}' > $vs\_NIR_RAS_p$pf.txt `;
		`perl /users/ablife/ablife-perl/public/get_sub_file_filter_by_titleitem.pl _$vs\_coir_test_result.txt 'ID|Chr|Ass|Strand|Mss|Type|Isoform|KorN|$samplelist1|$samplelist2|Tvalue|Pvalue' > _$vs\_ir_test_result.txt && cat _$vs\_ir_test_result.txt | perl -ne 'chomp;if(/^X\\.ID/){print \$_,"\\tDiffType\\n";next;}\@line=split(/\\t/);if(\$line[-2]<=$pf && \$line[-1]<0){print \$_,"\\tdown\\n";}if(\$line[-2]<=$pf && \$line[-1]>0){print \$_,"\\tup\\n";}' > $vs\_IR_RAS_p$pf.txt `;
	}
	chdir($outdir);
}

sub make_ras_test_script {
	my ($list1,$list2,$rfilename,$dir)=@_;
...
sub Co_RAS_analysis {    #&Select_co_RAS($method,$outdir,$file1,$file2,$i,$j,\%RAS_method)
	...
	foreach my $coRAS (sort keys %{$option_ref->{"CORAS"}}) {
		...
		# `Rscript /users/ablife/ablife-R/Venn/latest/Venn_notitle.r -f $venn_f -n $coRAS_name\_co_RAS_venn`;
	}
...
sub Do_diras_nir {
	...
	my $venn_f = `ls *_nir_up | cat | perl -ne 'chomp;print \$_,",";'|sed 's/,\$//'`;
	`Rscript /users/ablife/ablife-R/Venn/latest/Venn_notitle.r -f $venn_f -n diRAS_NIR_up_venn`;
	$venn_f = `ls *_nir_down | cat | perl -ne 'chomp;print \$_,",";'|sed 's/,\$//'`;
	`Rscript /users/ablife/ablife-R/Venn/latest/Venn_notitle.r -f $venn_f -n diRAS_NIR_down_venn`;
...
sub Do_diras_ir {
	...
	my $venn_f = `ls *_ir_up | cat | perl -ne 'chomp;print \$_,",";'|sed 's/,\$//'`;
	`Rscript /users/ablife/ablife-R/Venn/latest/Venn_notitle.r -f $venn_f -n diRAS_IR_up_venn`;
	$venn_f = `ls *_ir_down | cat | perl -ne 'chomp;print \$_,",";'|sed 's/,\$//'`;
	`Rscript /users/ablife/ablife-R/Venn/latest/Venn_notitle.r -f $venn_f -n diRAS_IR_down_venn`;
