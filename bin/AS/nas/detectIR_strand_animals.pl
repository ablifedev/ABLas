...
my %opts;
GetOptions(
    \%opts,  "bam=s", "as=s", "fa=s", "chrlen=s", "gff=s",
    "mod=s", "o=s",   "h"
);
...
#chr1    hg18_knownGene  exon    1320637 1320780 0.000000        -       .       ID=exon:CCNL2.1.5:2;Parent=CCNL2.1.5
        open OUT, ">$out_file\.chr_$chr"     || die;
        ...
        while (<GFF>) {
            ...
            # next
            #   if ( ( not defined( $gene_model{$gene} ) )
            #     || ( $isoform ne $gene_model{$gene} ) );
            ...
            if (   $isoform ne $forward_isoform
                ...
                && $exon_number > 1 )
            {
                # print $forward_chr,"\t",$forward_gene,"\t$gene\t$exon_number\n";
...
`cat $out_file\.chr_* >> $out_file && rm -rf $out_file\.chr_*`;
`cat $out_file\.log_* >> $out_file\.log && rm -rf $out_file\.log_*`;
