...
sub load_AS {
	...
	while (<AS>) {
		...
		if($type eq "ES"){
			...
			next if ($alt_avg_sj+$model_avg_sj==0);
			my $alt_ratio = sprintf("%.2f",$alt_avg_sj/($alt_avg_sj+$model_avg_sj));

			print OUT join("\t",@line[0..6]),"\t",$alt_ratio,"\t",$alt_avg_sj,"\t",$model_avg_sj,"\t",$alt_sj,"\t",$model_sj,"\t",$line[-1],"\n";
		}
