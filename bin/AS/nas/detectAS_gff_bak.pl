...
my $buf = "-";
my %opts;
GetOptions( \%opts, "asj=s", "gff=s", "mod=s", "sj=s", "o=s", "h" );
	...
	|| defined( $opts{h} ) )
{
	...
	Usage:perl $0
		...
		-asj        annotated SJ           must be given;
...
while (<SJ>) {              #把所有的junction添加进hash
	...
	chomp;
	next if ( $_ =~ /^track/i );
	my ( $chr, $left, $right, $feature, $readsNum, $strand) = split /\s+/ ;
...
sub handling_gene {
	...
	foreach my $point (@sort_keys) {
		...
		next if ( @start_junction < 1 && @end_junction < 1 );
