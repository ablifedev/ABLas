...
while (<GFF>) {
	...
	if ( $feature =~ /^(mRNA|miRNA|mRNA_TE_gene|ncRNA|rRNA|snoRNA|snRNA|tRNA|transcript)$/ ) {
		...
		&handling_gene( \%exon, \@exon_hash, \@intron_hash, \%gene_region, $this_strand, $this_chr, $gene, $forward_start, $forward_end) if($gene ne "");
		$info =~ m/^ID=([\w|\.|\%|\:|\_|\-|\s]+);/;
		$gene = $1;
		$this_chr = $chr;
		$this_strand = $strand;
...
sub handling_gene{
	...
	LABEL1:foreach my $index (sort {$a<=>$b} keys %{$gene_region}) {
		...
		my $intron_base = &sum($chr, $intron_start, $intron_end,\%base);
		my $intron_max_base = &max_depth($chr, $intron_start, $intron_end,\%base);
