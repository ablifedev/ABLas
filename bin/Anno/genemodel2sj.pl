...
my $buf = "-";
my %opts;
GetOptions( \%opts, "gff=s", "mod=s", "o=s", "h" );
...
foreach my $chr (sort keys %gff){
	foreach my $strand (sort keys %{$gff{$chr}}){
		foreach my $gene (sort keys %{$gff{$chr}->{$strand}}){
			next if not defined($gene_model{$gene});
			my @thisgene = @{$gff{$chr}->{$strand}->{$gene}};
			# print $gene,"\n";
			&gene2sj($chr,$strand,\@thisgene,$gene);   ##find Mutually exclusive exons events
		}
	}
}
...
sub gene2sj {
	my ($chr,$strand,$thisgene,$genename) = @_;
	...
	if($strand eq "+"){
		for(my $f=0;2*$f+1<$#genesite;$f++){
			...
			if(defined($sj{$chr}->{$strand}->{$junc})){
				...
				print OUT "$chr\t$junc\t$strand\t$genename\n";
			}
		}
	}
	if($strand eq "-"){
