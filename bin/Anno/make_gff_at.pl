...
while (<IN>) {
	...
	#	}
	if($line[2] =~ /^mRNA|transcript$/){
		$line[8] =~ m/ID=([\w\:\.\-]+);.*Parent=([\w\:\.\-]+)/;
		$isoform="$1";
		$gene="$2";
	}
	if($line[2] =~ /^exon$/){
	...
	# if ( $line[2] =~ /^mRNA$/ ) {
	# 	print OUT "$line[0]\tmRNA\t$line[6]\t$line[3]\t$line[4]\t$isoform\n";
	# }
	if ( $line[2] =~ /^exon$/ ) {
		print OUT "$line[0]\texon\t$line[6]\t$line[3]\t$line[4]\t$isoform\n";
	}
	if ( $line[2] =~ /^intron$/ ) {
		print OUT "$line[0]\tintron\t$line[6]\t$line[3]\t$line[4]\t$isoform\n";
	}
	# if ( $line[2] =~ /five/ ) {
	# 	print OUT "$line[0]\tfive\t$line[6]\t$line[3]\t$line[4]\t$isoform\n";
	# }
	# if ( $line[2] =~ /three/ ) {
	# 	print OUT "$line[0]\tthree\t$line[6]\t$line[3]\t$line[4]\t$isoform\n";
	# }
