...
foreach my $asname(@name_group){
	print OUT "\t$asname\:totalbase,avg_depth,mid_depth\t$asname\:sj_reads\t$asname\:avg_depth\t$asname\:avg_sj_reads\t$asname\:Alt_ratio";
}
print OUT "\n";
...
for(my $i=0;$i<=$#file_group;$i++){
	...
	while (<AS>) {              #把所有的junction添加进hash
		...
		my $modle_avg=$line[7];
		my $value = "$line[5]\t$line[7]\t$avg_base\t$modle_avg\t$line[9]";
