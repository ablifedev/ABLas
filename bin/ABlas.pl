...
open OUT_CE, ">$out_file\_CE" if $gffsj==0;
		...
		for(my $f=0;2*$f<$#genesite-3;$f++){
			...
			if ( defined( $positive_start{$chr}->{$genesite[2*$f+1]} ) ){
				...
				foreach my $i (@start_junction) {
					my $position_left = $i;
					my $reads_left = $positive_sj{$chr}->{"$genesite[2*$f+1]:$position_left"};
					if($position_left>$genesite[2*$f+2] && $position_left<$genesite[2*$f+3]){
						if ( defined( $positive_end{$chr}->{$genesite[2*$f+4]} ) ){
							...
							foreach my $j (@end_junction) {
								my $position_right = $j;
								my $reads_right = $positive_sj{$chr}->{"$position_right:$genesite[2*$f+4]"};
	...
	if($strand eq "-"){
		for(my $f=0;2*$f<$#genesite-3;$f++){
			...
			if ( defined( $negative_start{$chr}->{$genesite[2*$f+1]} ) ){
				...
				foreach my $i (@start_junction) {
					my $position_left = $i;
					my $reads_left = $negative_sj{$chr}->{"$genesite[2*$f+1]:$position_left"};
					if($position_left<$genesite[2*$f+2] && $position_left>$genesite[2*$f+3]){
						if ( defined( $negative_end{$chr}->{$genesite[2*$f+4]} ) ){
							...
							foreach my $j (@end_junction) {
								my $position_right = $j;
								my $reads_right = $negative_sj{$chr}->{"$position_right:$genesite[2*$f+4]"};
								if($position_right<$genesite[2*$f+2] && $position_right>$genesite[2*$f+3]){
									next if $position_right>=$position_left;  ##有交叉
									if($gffsj==1){
										next if not defined($exon_hash{$chr}->{$strand}->{"$position_left:$position_right"});
		...
		for(my $f=0;2*$f<$#genesite;$f++){
			for(my $t = $genesite[2*$f];$t<=$genesite[2*$f+1];$t++){
				if ( defined( $positive_start{$chr}->{$t} ) ){
					...
					foreach my $i (@start_junction) {
						my $position = $i;
						my $reads = $positive_sj{$chr}->{"$t:$position"};
		...
		for(my $f=0;2*$f<$#genesite;$f++){
			for(my $t = $genesite[2*$f];$t>=$genesite[2*$f+1];$t--){
				if ( defined( $negative_start{$chr}->{$t} ) ){
					...
					foreach my $i (@start_junction) {
						my $position = $i;
						my $reads = $negative_sj{$chr}->{"$t:$position"};
						if($position<=$genesite[2*$f] && $position>=$genesite[2*$f+1]){
							my $flanks_left = $genesite[2*$f];
							my $flanks_right= $genesite[2*$f+1];
							my $structure = "1\^2\-\,0";
				...
				for(my $t = $genesite[2*$f+1]+1;$t<$genesite[2*$f+2];$t++){
					...
					if ( defined( $positive_start{$chr}->{$t} ) ){
						...
						foreach my $i (@start_junction) {
							my $position = $i;
							my $reads = $positive_sj{$chr}->{"$t:$position"};
							if($position>$genesite[2*$f+1] && $position<$genesite[2*$f+2] ){
								my $flanks_left = $genesite[2*$f];
								my $flanks_right= $genesite[2*$f+3];
								my $structure = "2\^3\-\,1\^4\-";
								my $splice_chain = "$t\^$position\-\,$genesite[2*$f+1]\^$genesite[2*$f+2]\-";
		...
		for(my $f=1;2*$f<$#genesite;$f++){
			...
			if ( defined( $positive_end{$chr}->{$junc_end} ) ){
				...
				LABEL1:foreach my $i (@start_junction) {
					my $position = $i;
					my $reads = $positive_sj{$chr}->{"$position:$junc_end"};
					my $junc = "$position:$junc_end";
					my $junc2 = "$genesite[2*$f-1]:$junc_end";
					next if defined($usedjunc{$chr}->{"+"}->{"$junc:$junc2"});
					...
					my $splice_position = "";
					my $alt_exon_start = "";
					if($position<$genesite[2*$f-1]){
						$structure = "$splice_site\^\,".($splice_site+1)."\^";
						$splice_chain = "$position\^\,".$genesite[2*$f-1]."\^";
						$splice_position = "$position,$genesite[2*$f-1]";
						my $info = "flanks \"$flanks_left\^\,$flanks_right\-\"\; structure \"$structure\"\; splice_chain \"$splice_chain\"\; flank_exon_start \"$flank_exon_start\"\; flank_exon_end \"$flank_exon_end\"\; splice_position \"$splice_position\"\;";
						print OUT_A5SS "$chr\t$gene\tA5SS\t$position\t$flanks_right\t\.\t$strand\t$reads\t$info\n";
					}elsif($position>$genesite[2*$f-1]){
						$structure = "2\^\,"."1"."\^";
						$splice_chain = "$position\^\,".$genesite[2*$f-1]."\^";
						$splice_position = "$genesite[2*$f-1],$position";
						my $info = "flanks \"$flanks_left\^\,$flanks_right\-\"\; structure \"$structure\"\; splice_chain \"$splice_chain\"\; flank_exon_start \"$flank_exon_start\"\; flank_exon_end \"$flank_exon_end\"\; splice_position \"$splice_position\"\;";
						print OUT_A5SS "$chr\t$gene\tA5SS\t$genesite[2*$f-1]\t$flanks_right\t\.\t$strand\t$reads\t$info\n";
					}
				}
			}
		}
	...
	if($strand eq "-"){
		for(my $f=1;2*$f<$#genesite;$f++){
			...
			if ( defined( $negative_end{$chr}->{$junc_end} ) ){
				...
				LABEL1:foreach my $i (@start_junction) {
					my $position = $i;
					my $reads = $negative_sj{$chr}->{"$position:$junc_end"};
					my $junc = "$position:$junc_end";
					my $junc2 = "$genesite[2*$f-1]:$junc_end";
					next if defined($usedjunc{$chr}->{"-"}->{"$junc:$junc2"});
					next if $position==$genesite[2*$f-1];
		...
		for(my $f=0;2*$f+1<$#genesite;$f++){
			...
			if ( defined( $positive_start{$chr}->{$junc_start} ) ){
				...
				LABEL1:foreach my $i (@end_junction) {
					my $position = $i;
					my $reads = $positive_sj{$chr}->{"$junc_start:$position"};
					my $junc = "$junc_start:$position";
					my $junc2 = "$junc_start:$genesite[2*$f+2]";
					next if defined($usedjunc{$chr}->{"+"}->{"$junc:$junc2"});
					...
					my $splice_position = "";
					my $alt_exon_end = "";
					if($position<$genesite[2*$f+2]){
						$structure = "$splice_site\-\,".($splice_site+1)."\-";
						$splice_chain = "$position\-\,".$genesite[2*$f+2]."\-";
						$splice_position = "$position,$genesite[2*$f+2]";
						my $info = "flanks \"$flanks_left\^\,$flanks_right\-\"\; structure \"$structure\"\; splice_chain \"$splice_chain\"\; flank_exon_start \"$flank_exon_start\"\; flank_exon_end \"$flank_exon_end\"\; splice_position \"$splice_position\"\;";
						print OUT_A3SS "$chr\t$gene\tA3SS\t$flanks_left\t$genesite[2*$f+2]\t\.\t$strand\t$reads\t$info\n";
					}elsif($position>$genesite[2*$f+2]){
						# $structure = "$splice_site\-\,".($splice_site+1)."\-";
						$structure = "2\-\,"."1"."\-";
						# $splice_chain = $genesite[2*$f+2]."\-\,"."$position\-";
						$splice_chain = "$position\-\,".$genesite[2*$f+2]."\-";
						$splice_position = "$genesite[2*$f+2],$position";
						my $info = "flanks \"$flanks_left\^\,$flanks_right\-\"\; structure \"$structure\"\; splice_chain \"$splice_chain\"\; flank_exon_start \"$flank_exon_start\"\; flank_exon_end \"$flank_exon_end\"\; splice_position \"$splice_position\"\;";
	...
	if($strand eq "-"){
		for(my $f=0;2*$f+1<$#genesite;$f++){
			...
			if ( defined( $negative_start{$chr}->{$junc_start} ) ){
				...
				LABEL1:foreach my $i (@end_junction) {
					my $position = $i;
					my $reads = $negative_sj{$chr}->{"$junc_start:$position"};
					my $junc = "$junc_start:$position";
					my $junc2 = "$junc_start:$genesite[2*$f+2]";
					next if defined($usedjunc{$chr}->{"-"}->{"$junc:$junc2"});
					next if $position==$genesite[2*$f+2];
...
sub find_afe_ale {
	...
	if($strand eq "+"){
		if ( defined( $positive_end{$chr}->{$genesite[2]} ) ){
			...
			foreach my $j (@end_junction) {
				my $position = $j;
				my $reads = $positive_sj{$chr}->{"$position:$genesite[2]"};
				if($position<$genesite[1]){
					...
					my $splice_chain = "unknown\[$position\^,unknown\[$genesite[1]\^";
					my $splice_position = "unknown,$position,unknown,$genesite[1]";
					...
					my $junc2 = "$genesite[1]:$genesite[2]";
					$usedjunc{$chr}->{"+"}->{"$junc:$junc2"} = 1;
					$usedjunc{$chr}->{"+"}->{"$junc2:$junc"} = 1;
				}
			}
		}
		if ( defined( $positive_start{$chr}->{$genesite[-3]} ) ){
			...
			foreach my $j (@start_junction) {
				my $position = $j;
				my $reads = $positive_sj{$chr}->{"$genesite[-3]:$position"};
				if($position>$genesite[-2]){
					...
					my $splice_chain = "$position\-unknown\],$genesite[-2]\-unknown\]";
					my $splice_position = "$genesite[-2],unknown,$position,unknown";
					my $info = "flanks \"$flanks_left\^\,$flanks_right\-\"\; structure \"$structure\"\; splice_chain \"$splice_chain\"\; flank_exon_start \"$flank_exon_start\"\; flank_exon_end \"$flank_exon_end\"\; alt_exon_start \"$alt_exon_start\"\; alt_exon_end \"$alt_exon_end\"\; splice_position \"$splice_position\"\;";
					print OUT_ALE "$chr\t$gene\tALE\t$flanks_left\t$position\t$model_sj_reads\t$strand\t$reads\t$info\n";
					my $junc = "$genesite[-3]:$position";
					my $junc2 = "$genesite[-3]:$genesite[-2]";
					$usedjunc{$chr}->{"+"}->{"$junc:$junc2"} = 1;
					$usedjunc{$chr}->{"+"}->{"$junc2:$junc"} = 1;
				}
			}
	...
	if($strand eq "-"){
		if ( defined( $negative_end{$chr}->{$genesite[2]} ) ){
			...
			foreach my $j (@end_junction) {
				my $position = $j;
				my $reads = $negative_sj{$chr}->{"$position:$genesite[2]"};
				if($position>$genesite[1]){
					#flanks "null,831045-"; structure "1[2^,3[4^"; splice_chain "830120[830204^,830375[830922^";AFE
					my $flanks_left = "null";
					my $flanks_right= $genesite[2];
		...
		if ( defined( $negative_start{$chr}->{$genesite[-3]} ) ){
			...
			foreach my $j (@start_junction) {
				my $position = $j;
				my $reads = $negative_sj{$chr}->{"$genesite[-3]:$position"};
				if($position<$genesite[-2]){
					#flanks "831045^,null"; structure "3-4],1-2]"; splice_chain "830120-830204],830375-830922]";ALE
					my $flanks_left = $genesite[-3];
					my $flanks_right= "null";
	...
	if($strand eq "+"){
		for(my $f=0;2*$f+1<$#genesite-2;$f++){
			...
			if ( defined( $positive_start{$chr}->{$junc_start} ) && defined( $positive_end{$chr}->{$junc_end} ) ){
				...
				foreach my $i (@start_junction) {
					my $position_left = $i;
					my $reads_left = $positive_sj{$chr}->{"$junc_start:$position_left"};
					next if $position_left>= $junc_end; #junction的终点越过了下一个exon的起点
					foreach my $j (@end_junction) {
						my $position_right = $j;
						my $reads_right = $positive_sj{$chr}->{"$position_right:$junc_end"};
	...
	if($strand eq "-"){
		for(my $f=0;2*$f+1<$#genesite-2;$f++){
			...
			if ( defined( $negative_start{$chr}->{$junc_start} ) && defined( $negative_end{$chr}->{$junc_end} ) ){
				...
				foreach my $i (@start_junction) {
					my $position_left = $i;
					my $reads_left = $negative_sj{$chr}->{"$junc_start:$position_left"};
					next if $position_left<= $junc_end; #junction的终点越过了下一个exon的起点
					foreach my $j (@end_junction) {
						my $position_right = $j;
						my $reads_right = $negative_sj{$chr}->{"$position_right:$junc_end"};
						next if $position_right>=$junc_start; #junction的起点越过了上一个exon的终点
						next if $position_right>=$position_left; #两个junction有交叉
						if($gffsj==1){
							next if not defined($exon_hash{$chr}->{$strand}->{"$position_left:$position_right"});
			...
			if ( defined( $positive_start{$chr}->{$junc_start} ) && defined( $positive_end{$chr}->{$junc_end} ) ){
				...
				foreach my $i (@start_junction) {
					...
					foreach my $j (@end_junction) {
						...
						my $junc = "$junc_start:$junc_end";
						my $junc2 = "$junc_start:$position_left";
						$usedjunc{$chr}->{"+"}->{"$junc:$junc2"} = 1;
						$usedjunc{$chr}->{"+"}->{"$junc2:$junc"} = 1;
						$junc = "$junc_start:$junc_end";
						$junc2 = "$position_right:$junc_end";
						$usedjunc{$chr}->{"+"}->{"$junc:$junc2"} = 1;
						$usedjunc{$chr}->{"+"}->{"$junc2:$junc"} = 1;
					}
				}
					...
					for(my $r=$f+2;2*$r<$#genesite;$r++){
						if($position==$genesite[2*$r]){
							...
							for(my $m=$f+1;$m<$r;$m++){
								...
								my $junc2 = "$junc_start:$position";
								$usedjunc{$chr}->{"+"}->{"$junc:$junc2"} = 1;
								$usedjunc{$chr}->{"+"}->{"$junc2:$junc"} = 1;
								$splice_site+=2;
							}
							my $junc = "$genesite[2*$r-1]:$genesite[2*$r]";
							my $junc2 = "$junc_start:$position";
							$usedjunc{$chr}->{"+"}->{"$junc:$junc2"} = 1;
							$usedjunc{$chr}->{"+"}->{"$junc2:$junc"} = 1;
							$splice_position=~s/^,//;
							...
							print OUT_SE "$chr\t$gene\tSE\t$flanks_left\t$flanks_right\t$reads\t$strand\t$inclusive_sj_reads\t$info\n";
							# my $junc = "$junc_start:$position";
							# $usedjunc{$chr}->{"+"}->{$junc} = 1;
						}
					}
...
sub load_sj {
	...
	while (<SJ>) {              #把所有的junction添加进hash
		...
		if ( $strand eq "+" ) {
			...
			next if defined $sj{$chr}{"$start:$end"};
			next if $positive_sj{$chr}->{"$start:$end"}<4;
			$sj{$chr}{"$start:$end"}="";
			if ( !defined( $positive_start{$chr}->{$start} ) ) {
				$positive_start{$chr}->{$start} = "$end";
			}
			else {
				$positive_start{$chr}->{$start} .= "\t$end";
			...
			$end   = $left + $leftsize;
			next if defined $sj{$chr}{"$start:$end"};
			next if $negative_sj{$chr}->{"$start:$end"}<4;
			$sj{$chr}{"$start:$end"}="";
			if ( !defined( $negative_start{$chr}->{$start} ) ) {
				$negative_start{$chr}->{$start} = "$end";
			}
			else {
				$negative_start{$chr}->{$start} .= "\t$end";
			}
			if ( !defined( $negative_end{$chr}->{$end} ) ) {
				$negative_end{$chr}->{$end} = "$start";
			}
			else {
				$negative_end{$chr}->{$end} .= "\t$start";
			}
...
sub load_all_sj {
	...
	while (<SJ>) {              #把所有的junction添加进hash
		...
		if ( $strand eq "+" ) {
			$start = $left + $leftsize;
			$end   = $right - $rightsize + 1;
			if(not defined($positive_sj{$chr}->{"$start:$end"})){
...
sub load_all_gffsj {
	...
	while (<SJ>) {              #把所有的junction添加进hash
		...
		my ( $chr, $start, $end, $strand) = split /\t/ ;
		if ( $strand eq "+" ) {
			$positive_sj{$chr}->{"$start:$end"} = 50;
		}
		else {
			$negative_sj{$chr}->{"$start:$end"} = 50;
		}
	}
	close SJ;
	#调试信息
