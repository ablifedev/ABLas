...
while (<GFF>) {
	...
	next if ( $feature =~ /^(chromosome|gene)$/ || $_ =~ /^transposable_element_gene$/);
...
sub handling_gene{
	...
	LABEL1:foreach my $index (sort {$a<=>$b} keys %{$gene_region}) {
		...
		next LABEL1 if $intron_mid_base < 2;
		...
		next LABEL1 if($intron_mid_base < $ends_exon_ave_base * 0.2 && $intron_ave_base < 20);
