...
while (<CHRLEN>) {
	...
	if($line[0] ne $forward_chr && $forward_chr ne ""){
		print OUT "perl5.14 $Bin/ABlas_IntroR_unstrand_human.pl -bam $bam_file -fa $fa_file -chrlen $chrlen_file -gff $gff_file -o $out_file"."_$forward_chr"." -anno $anno_file -od $outdir -chr $forward_chr && \n";
	}
	$forward_chr = $line[0];
}
close CHRLEN;
print OUT "perl5.14 $Bin/ABlas_IntroR_unstrand_human.pl -bam $bam_file -fa $fa_file -chrlen $chrlen_file -gff $gff_file -o $out_file"."_$forward_chr"." -anno $anno_file -od $outdir -chr $forward_chr &&\n";
`perl /public/bin/qsub-sge.pl --queue all.q --resource vf=10.0G --maxproc 160 run_intronR.sh`;
